...
//Seek to location in file and write
void thread_fs_sw(void *arg){

        struct thread_arg *t_arg = arg;
        char text[100]; //user input limited to 100 for now
        char *diskname, *filename;
        int fs_fd;
        int stat;

        if (t_arg->argc < 2)
                die("need <diskname> <filename> <text>");

        diskname = t_arg->argv[0];
        filename = t_arg->argv[1];
        //text = t_arg->argv[2];
        strcpy(text,t_arg->argv[2]);
        ...
        written = fs_write(fs_fd, text, strlen(text));
...
//Seek to location in file and write
void thread_fs_emptyoffsetWrite(void *arg){
...
static struct {
        ...
        { "cat",  thread_fs_cat },
        { "stat", thread_fs_stat },
        { "seekwrite", thread_fs_sw},
        { "seekempty", thread_fs_emptyoffsetWrite}
};
